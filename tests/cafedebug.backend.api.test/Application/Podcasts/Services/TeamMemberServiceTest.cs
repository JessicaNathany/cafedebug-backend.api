...
public class TeamMemberServiceTest : BaseTest
{
    ...
    public async Task GetAllAsync_WithValidRequest_ReturnsSuccess()
    {
      ...
      var teamMembers = _teamMemberTestDataMock.CreateTeamMembers(request.Page, request.PageSize, request.SortBy, request.Descending);
      ...
      _teamMemberRepositoryMock.Verify(x => x.GetPageList(request.Page, request.PageSize, request.SortBy, request.Descending, CancellationToken.None), Times.Once());
    }
}