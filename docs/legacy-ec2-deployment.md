> **Legacy reference only.** Production now uses Railway. The GitHub image publishing, release deployment, and EC2 rollback workflows described below have been retired. These historical instructions are not the current deployment procedure. See [Deployment](../DEPLOYMENT.md). Existing resources and images are not removed by this change.
